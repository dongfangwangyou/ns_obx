...
本文档描述 ns_obx 内部依赖追踪与响应式数据流设计，面向维护者与需要深入理解 Obx / Rx 行为的进阶用户。API
用法见 [README_CN.md](README_CN.md)。
...
`bindStream`、`select` 对父 Rx 的 listen 等，通过 `ReactiveMixin.linkSubscription` 登记在
`_linkedSubscriptions`，**不进入** `_dependencies`。
...
同一帧内多个 Rx 连续 `subject.add` 时，`ObxObserver._dispatch` 通过
`SchedulerBinding.scheduleFrameCallback` 将多次通知合并为 **一次** `setState`。
...
集合写操作走 `rawValue` / 内部路径，读操作用 `readTracked` 或 `ReactiveMixin.value`，避免重复注册 Obx
依赖。
...
- 不触发 Obx 式依赖注册
- 不受 Obx sweep 影响
- 需在页面 dispose 时 `worker.dispose()` 或配合 `RxLifecycleMixin`
...
| `RxLifecycleMixin`    | Widget dispose 时自动 `close` 注册的 Rx / 订阅 / Worker              |

---

## 12. 1.0.3 API 收窄

- 移除 **`RxInterface.proxy` public setter**；生产压栈走私有 **`_buildWithProxy`**，`notifyDependents`
  在其上附加 `canUpdate` 断言
...
- [README_CN.md](README_CN.md) — 安装、API、示例
- [CHANGELOG.md](CHANGELOG.md) — 版本变更
- [example/](example/lib/main.dart) — 可运行 Demo（含 **Obx** Tab）
